...
type TeamRepository interface {
	// List retorna as equipes de uma liga. Quando uma ou mais temporadas são
	// informadas, filtra para equipes que de fato jogaram naquela liga naquelas
	// temporadas (via matches) — evita listar equipes de temporadas passadas (ex:
	// rebaixadas) como se ainda estivessem na liga atual. Sem temporada, cai no
	// vínculo histórico geral (league_teams), usado para "todas as temporadas".
	List(ctx context.Context, leagueID *int64, seasonIDs ...int64) ([]domain.Team, error)
