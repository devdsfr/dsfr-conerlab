...
func (r *TeamRepo) List(ctx context.Context, leagueID *int64, seasonIDs ...int64) ([]domain.Team, error) {
	query := `SELECT id, name, short_name, country, tier, created_at FROM teams ORDER BY name`
	args := []any{}
	switch {
	case leagueID != nil && len(seasonIDs) > 0:
		// Restringe a equipes que de fato têm partida registrada nessa liga nas
		// temporadas pedidas — league_teams sozinho é um vínculo histórico "alguma
		// vez jogou aqui" e listava equipes de temporadas passadas (ex: rebaixadas)
		// na temporada atual. season_id = ANY(...) cobre o multi-select do Simulador.
		query = `
			SELECT DISTINCT t.id, t.name, t.short_name, t.country, t.tier, t.created_at
			FROM teams t
			JOIN matches m ON (m.home_team_id = t.id OR m.away_team_id = t.id)
			WHERE m.league_id = $1 AND m.season_id = ANY($2)
			ORDER BY t.name`
		args = append(args, *leagueID, seasonIDs)
	case leagueID != nil:
		query = `
